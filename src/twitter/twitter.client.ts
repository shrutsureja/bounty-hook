...
async function generateOAuth2AuthLink({
  ...
  const response = await fetch(
    `https://twitter.com/i/oauth2/authorize?response_type=${'code'}&client_id=${clientId}&redirect_uri=${callbackUrl}&scope=${finalScope}&state=${state}&code_challenge=${codeChallenge}&code_challenge_method=${code_challenge_method}`
  );
  // For Debugging
